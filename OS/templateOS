...
local function drawText(text, pos, scale, co)
	...
	for i = 1, #text do
		...
		local bitmap = require(script.Parent.Parent.FONTS.Bitmap)[string.upper(c)]
		if bitmap then
			for j = 1, #bitmap do
				for k = 1, #bitmap[j] do
...
module.load = function(r,lR,p)
	...
	drawText("TEMPLATE OS",Vector2.new(10,10),1,Color3.new(1, 0.894118, 0.894118))
end
